...
const gameLogic = (() => {
  ...
  const initializeGame = () => {
    ...
    plrs.push(Player("Ganz", "rgb(134, 235, 142)"));
    plrs.push(Player("Opponent", "rgb(250, 145, 145)"));
    _currentPlr = plrs[0];
  ...
  const checkForConnectedPieces = (plr) => {
    ...
    for (let row = 1; row <= rowCount; row++) {
      ...
      for (let col = 1; col <= colCount; col++) {
        ...
        if (isCurrentCellPlrColour) {
          col == colCount ? rowScore += scoringFormula(connectedPieces + 1) : connectedPieces++
        } else {
          if (connectedPieces >= 3) { rowScore += scoringFormula(connectedPieces) };
          connectedPieces = 0
        }
      }
      totalScore += rowScore
    ...
    for (let col = 1; col <= colCount; col++) {
      ...
      for (let row = 1; row <= rowCount; row++) {
        ...
        let isCurrentCellPlrColour = currentCell.getAttribute("colour") == plr.getMarkerColour();
        if (isCurrentCellPlrColour) {
          row == rowCount ? colScore += scoringFormula(connectedPieces + 1) : connectedPieces++
        } else {
          if (connectedPieces >= 3) { colScore += scoringFormula(connectedPieces) };
          connectedPieces = 0
        }
      }
      totalScore += colScore
    ...
    console.log(`${plr.getName()} has ${totalScore}`);
  }

  const _playRound = (plr, selectedCell) => {
    if (!(gameActive)) { return } // <-- do completed game stuff
