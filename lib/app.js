...
const gameEndedSection = document.querySelector(".game-ended-section");
const returnToMenuBtn = document.querySelector(".top-bar > button");
const playAgainBtn = document.querySelector(".game-ended-buttons > button:first-child");
const menuBtn = document.querySelector(".game-ended-buttons > button:last-child");
...
const gameBoard = (() => {
  ...
  const _getBoardInfo = () => {
    ...
    return { cellSize, containerHeight, containerWidth, rowsAmt, colsAmt, cellGap };
  }

  const generateGameboard = () => {
    _deleteOldBoardCells();
    const { cellSize, containerHeight, containerWidth, rowsAmt, colsAmt, cellGap } = _getBoardInfo();
    gameboardContainer.setAttribute("style", `width: ${containerWidth}px; height: ${containerHeight}px; grid-template: repeat(${rowsAmt}, ${cellSize}px) / repeat(${colsAmt}, ${cellSize}px); gap: ${cellGap}px`);
...
const gameLogic = (() => {
  ...
  const _generateNewGameData = () => {
    ...
    _displayScores();
    plrScoreboards[0].style.backgroundColor = plrs[0].getMarkerColour();
    plrScoreboards[1].style.backgroundColor = "rgb(226, 226, 226)";
    ...
    plrScoreboards[0].classList.add("current-turn");
    plrScoreboards[1].classList.remove("current-turn");
    gameEndedSection.classList.add("inactive");
  }

  const initializeGame = () => {
    _generateNewGameData();
    _currentPlr = plrs[0];
  ...
  const _setCurrentPlayer = () => {
    ...
    plrScoreboards.forEach(scoreboard => {
      scoreboard.classList.toggle("current-turn");
      scoreboard.classList.contains("current-turn") ? scoreboard.style.backgroundColor = _currentPlr.getMarkerColour()
                                                    : scoreboard.style.backgroundColor = "rgb(226, 226, 226)";
    });
  }

  const displayWinner = () => {
    const playerWonText = document.querySelector(".player-won-text");
    if (plrs[0].getScore() == plrs[1].getScore()) {
      playerWonText.textContent = "The game was a tie!"
    } else {
      const winnerName = plrs[0].getScore() > plrs[1].getScore() ? plrs[0].getName() : plrs[1].getName();
      playerWonText.textContent = `${winnerName} won the game!`;
    }
    gameEndedSection.classList.remove("inactive");
  }
...
returnToMenuBtn.addEventListener("click", e => { UITransition.changeScreen(gameScreen, startMenuScreen) });
menuBtn.addEventListener("click", e => { UITransition.changeScreen(gameScreen, startMenuScreen) });
playAgainBtn.addEventListener("click", e => { gameLogic.initializeGame() });