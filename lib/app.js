...
const gameBoard = (() => {
  ...
  const cellTaken = selected_cell => {
    selected_cell.classList.contains("taken");
  }
...
const gameLogic = (() => {
  ...
  const _playRound = (plr, selectedTile) => {
    if (!(gameActive)) { return } // <-- do completed game stuff

    // run while game is incomplete
    if (!(gameBoard.cellTaken(selectedTile))) { // run while cell not taken
      console.log(selectedTile);
      selectedTile.classList.add("taken");
      console.log(selectedTile);
    }
  }
